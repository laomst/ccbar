...
### 3.2 下拉菜单与内联子按钮
- 点击快捷按钮后弹出选项菜单
- 每个 Option 行左侧显示选项名称，右侧**内联显示**所有子按钮
- 子按钮过多时自动换行显示
- **点击 Option 名称区域**：执行 Option.baseCommand（不带参数）
- **点击子按钮**：执行 Option.baseCommand + SubButton.params

```
┌──────────────────────────────────────┐
│ Model     [Default][Sonnet][Opus]    │  ← 子按钮内联显示
│ Workspace [Home][Work]               │  ← 点击 "Workspace" 文字执行裸命令
│ System    [Dev]                      │  ← 子按钮过多时自动换行
└──────────────────────────────────────┘
```
...
### 3.6 终端窗口管理
- 自动将终端作为编辑器 Tab 打开（而非底部工具栏终端）
- 自动固定终端标签页，防止意外关闭
- 支持全局设置：是否自动移动到编辑器区域、是否自动固定
- 不自动分屏，由用户自行决定分屏方式
...
**交互流程：**
1. 点击工具栏 `🤖 Claude` 按钮
2. 弹出下拉菜单，每个 Option 行右侧内联显示子按钮（如 [Default] [Sonnet] [Opus]）
3. 点击 Option 名称或子按钮
4. 弹出终端命名对话框，显示默认名称（可编辑）
5. 用户确认名称，点击 OK
6. 新建终端 Tab，自动在编辑器区域打开
7. 固定终端标签页（显示锁定图标）
8. 在终端中执行完整命令
...
│               ┌───────────────────────────────────────┐                          │
│               │ Model     [Default][Sonnet][Opus]     │  ← 子按钮内联显示        │
│               │ Workspace [🏠 Home][💼 Work]           │  ← 点击文字执行裸命令    │
│               │ System    [⚙️ Dev]                    │  ← 子按钮过多时换行      │
│               └───────────────────────────────────────┘                          │
│                     │                                                            │
│                     ▼ 点击 [Sonnet] 或点击 "Model"                               │
│               ┌─────────────────────────────┐                                    │
...
│  │ Editor Tabs                                                            │    │
│  │ ┌──────────┐ ┌──────────────────────────────┐                          │    │
│  │ │ Main.ts  │ │ Claude - Model - Sonnet 🔒   │  ← 固定标签页           │    │
│  │ └──────────┘ └──────────────────────────────┘                          │    │
│  │ ┌──────────────────────────────────────────────────────────────────┐   │    │
│  │ │ $ claude --model sonnet                                         │   │    │
│  │ │                                                                  │   │    │
...
说明：
- 每行 Option 右侧内联显示所有子按钮，子按钮过多时自动换行
- 子按钮可以是图标或文字标签
- 点击 Option 名称：执行 Option.baseCommand（不带参数）
- 点击子按钮：执行 Option.baseCommand + SubButton.params
- 点击后弹出命名对话框（每次都弹出）
- 默认名称格式：来自 Option.defaultTerminalName（点击 Option 或其下任何 SubButton 均使用此名称）
- 点击 OK 后新建终端，在编辑器区域打开为普通 Tab（不自动分屏）
- 终端标签页自动固定（显示锁定图标 🔒）
...
| 分类 | 设置项 | 说明 | 默认值 |
|------|--------|------|--------|
| Terminal Window | Open in editor area | 自动在编辑器区域打开终端 Tab | ✓ 选中 |
...
data class GlobalSettingsConfig(
    // 是否自动在编辑器区域打开终端 Tab
    var openInEditorArea: Boolean = true,
    // 是否自动固定终端标签页
...
| 终端打开到编辑器区域 | P0 | 在编辑器区域创建包含终端的自定义 Tab（不自动分屏） |
| 标签页固定 | P0 | 自动固定终端标签页 |
