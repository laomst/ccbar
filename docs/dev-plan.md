...
| ID | 任务 | 状态 | 依赖 | 产出文件 |
|----|------|------|------|---------|
| 1.4.1 | 定义默认配置常量（包含一个 "Claude Code" Button、"Model" Option、"Sonnet"/"Opus" SubButton） | `[ ]` | 1.3.1 | `settings/CCBarSettings.kt` |
...
| 2.1.2 | 实现 `CCBarButtonAction`，每个实例对应一个 `ButtonConfig`，`actionPerformed()` 中触发弹出菜单 | `[ ]` | 2.1.1 | `actions/CCBarButtonAction.kt` |
| 2.1.3 | 在 `plugin.xml` 中注册 `CCBarToolbarActionGroup` 到 `MainToolbarRight` | `[ ]` | 2.1.1 | `plugin.xml` |
| 2.1.4 | 按钮图标加载：先使用固定的内置图标（`AllIcons.Actions.Execute`），图标管理系统在 Phase 4 实现 | `[ ]` | 2.1.2 | `actions/CCBarButtonAction.kt` |
...
| ID | 任务 | 状态 | 依赖 | 产出文件 |
|----|------|------|------|---------|
| 2.2.1 | 实现 `CCBarPopupBuilder`，负责根据 `ButtonConfig` 构建弹出面板 | `[ ]` | 2.1.2 | `actions/CCBarPopupBuilder.kt` |
| 2.2.2 | 实现 Option 行布局：左侧 Option 名称（可点击 JLabel），右侧内联排列 SubButton（JButton） | `[ ]` | 2.2.1 | `actions/CCBarPopupBuilder.kt` |
| 2.2.3 | 实现子按钮过多时的自动换行布局（使用 `FlowLayout` 或 `WrapLayout`） | `[ ]` | 2.2.2 | `actions/CCBarPopupBuilder.kt` |
...
| 2.2.6 | 将弹出菜单集成到 `CCBarButtonAction.actionPerformed()`，点击按钮时显示在按钮下方 | `[ ]` | 2.1.2, 2.2.4 | `actions/CCBarButtonAction.kt` |
...
| ID | 任务 | 状态 | 依赖 | 产出文件 |
|----|------|------|------|---------|
| 2.3.1 | 实现终端命名对话框，使用 `Messages.showInputDialog()`，默认名称取自 `OptionConfig.defaultTerminalName` | `[ ]` | 1.2.3 | `terminal/CCBarTerminalService.kt` |
...
| ID | 任务 | 状态 | 依赖 | 产出文件 |
|----|------|------|------|---------|
| 3.4.1 | 使用 Kotlin UI DSL v2 构建 Button 详情表单：Name 文本框、Icon 文本框（Phase 4 升级为带浏览按钮） | `[ ]` | 3.3.3 | `settings/ui/CCBarSettingsPanel.kt` |
| 3.4.2 | 实现表单字段与选中 `ButtonConfig` 的双向绑定（选中切换时刷新，编辑时回写） | `[ ]` | 3.4.1 | `settings/ui/CCBarSettingsPanel.kt` |
...
| 4.1.3 | 将图标系统集成到工具栏按钮：`CCBarButtonAction` 使用 `CCBarIcons.loadIcon()` 加载配置中的图标 | `[ ]` | 4.1.1, 2.1.4 | `actions/CCBarButtonAction.kt` |
| 4.1.4 | 将图标系统集成到弹出菜单的 SubButton（如果 SubButton 配置了 icon） | `[ ]` | 4.1.1, 2.2.2 | `actions/CCBarPopupBuilder.kt` |
...
| ID | 任务 | 状态 | 依赖 | 产出文件 |
|----|------|------|------|---------|
| 4.2.1 | 将 Button 详情中的 Icon 文本框升级为 `TextFieldWithBrowseButton`，支持浏览选择 SVG/PNG 文件 | `[ ]` | 3.4.1, 4.1.1 | `settings/ui/CCBarSettingsPanel.kt` |
