...
object CCBarIcons {
     ...
     * 加载 IDEA 内置图标
     */
    private fun loadBuiltinIcon(fieldPath: String): Icon {
        return try {
            ...
            when (fieldPath) {
                "AllIcons.Actions.Execute" -> AllIcons.Actions.Execute
                "AllIcons.Actions.Compile" -> AllIcons.Actions.Compile
                "AllIcons.Actions.Refresh" -> AllIcons.Actions.Refresh
                "AllIcons.Actions.Cancel" -> AllIcons.Actions.Cancel
                "AllIcons.Actions.Close" -> AllIcons.Actions.Close
                ...
                else -> loadBuiltinIconByReflection(parts)
            }
        } catch (e: Exception) {
            LOG.warn("CCBar: 加载内置图标失败: $fieldPath", e)
            getDefaultIcon()
        }
    }
     ...
     * 通过反射加载内置图标
     */
    private fun loadBuiltinIconByReflection(parts: List<String>): Icon {
        return try {
            if (parts.size < 2) return getDefaultIcon()

            var targetClass: Class<*> = AllIcons::class.java

            for (i in 1 until parts.size - 1) {
                val className = parts[i]
                val found = targetClass.declaredClasses.find { it.simpleName == className }
                if (found != null) {
                    targetClass = found
                } else {
                    LOG.warn("CCBar: 未找到嵌套类: $className in ${targetClass.name}")
                    return getDefaultIcon()
                }
            }
            ...
            field.get(null) as? Icon ?: getDefaultIcon()
        } catch (e: Exception) {
            LOG.warn("CCBar: 反射加载内置图标失败: ${parts.joinToString(".")}", e)
            getDefaultIcon()
        }
    }
    ...
    fun getCommonBuiltinIcons(): List<Pair<String, String>> = listOf(
        ...
        "Run" to "builtin:AllIcons.Actions.Run",
        "Build" to "builtin:AllIcons.Actions.Build",
        "Compile" to "builtin:AllIcons.Actions.Compile",
        "Refresh" to "builtin:AllIcons.Actions.Refresh",
        "Gear" to "builtin:AllIcons.Actions.Gear",
        "Settings" to "builtin:AllIcons.General.Settings",
        "ToolWindowTerminal" to "builtin:AllIcons.ToolWindowToolWindow",
        "Console" to "builtin:AllIcons.Actions.Console",
